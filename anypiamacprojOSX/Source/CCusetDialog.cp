// $Id: CCusetDialog.cp,v 1.3 2007/11/07 13:06:57 djolson Exp $
//  CCusetDialog.cp
...
CCusetDialog *CCusetDialog::CreateCusetDialogStream(
   LStream *inStream )
{
   return new CCusetDialog( inStream );
}

//  Create a dialog from a stream

CCusetDialog::CCusetDialog( LStream *inStream ) :
   LDialogBox( inStream )
{ }

//  Destroy a dialog

CCusetDialog::~CCusetDialog()
{ }

//  Set menu status

void CCusetDialog::FindCommandStatus(
   CommandT  inCommand,
   Boolean   &outEnabled,
   Boolean&  /* outUsesMark */,
   UInt16&   /* outMark */,
...
void CCusetDialog::ListenToMessage(
   MessageT  inMessage,
   void      *ioParam )
{
   switch ( inMessage ) {
...
void CCusetDialog::FinishCreateSelf()
{
   LDialogBox::FinishCreateSelf();
   UReanimator::LinkListenerToControls( this, this,
      rRidL_CatchupSetDialog );
}

//  Save results from dialog

void CCusetDialog::SaveResults()
{
   ...
   try {
      for (int i = mCachup.getCstart(); i < mCachup.getCstart() + 10; i++) {
         for (int j = mCachup.getCstart() + 2;
            ...
            if (mCachup.get(i, j) !=
               theDoc->assumptions->catchupDoc.catchup.get(i, j)) {
               need_update = true;
            }
         }
      }
      if (need_update) {
         theDoc->assumptions->catchupDoc.setData(mAlt, mCachup);
      }
   } catch (PiaException& e) {
      messageDialog(e.getNumber(), this);
   } catch (...) {
      SignalStringLiteral_(
         "Exception caught in CCusetDialog::SaveResults");
   }
}

// Get string in a text box

string CCusetDialog::getTextBoxString( long textBoxID )
{
...
void CCusetDialog::setupdata()
{
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_EligyearTextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year1TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year2TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year3TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year4TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year5TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year6TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year7TextBox));
   ...
   theTextBox =
      dynamic_cast<LEditField *>(FindPaneByID(kStcD_Year8TextBox));
   LStr255 temp9(poutNoComma(mCuyear8, 2, 1).c_str());
   theTextBox->SetDescriptor(temp9);
   thePrevyearButton =
      dynamic_cast<LStdButton *>(FindPaneByID(kStcD_PrevyearButton));
   theNextyearButton =
      dynamic_cast<LStdButton *>(FindPaneByID(kStcD_NextyearButton));
   if (mEligyear <= mCachup.getCstart())
